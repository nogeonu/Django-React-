...
def save_as_dicom_seg(mask, output_path, reference_dicom_path, prediction_label="Tumor"):
    ...
    logger.info(f"  Final mask shape: {mask_frames.shape}")
    logger.info(f"  Number of source images: {len(source_images)}")
    ...
    seg_dataset = Segmentation(
        source_images=source_images,  # All frames
        pixel_array=mask_frames,      # All frames
    ...
    # 5. Save
    logger.info(f"  Final mask shape before saving: {mask_frames.shape}")
