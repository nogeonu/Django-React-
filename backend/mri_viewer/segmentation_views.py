...
@api_view(['GET'])
def get_segmentation_3d_data(request, seg_instance_id):
    """
    DICOM SEG 파일에서 종양 마스크 3D 배열과 spacing 정보를 추출하여 반환
    Plotly를 사용한 종양 전용 3D 렌더링을 위해 사용
    """
    try:
        logger.info(f"🔍 DICOM SEG → 3D 배열 추출 시작: {seg_instance_id}")
        client = OrthancClient(
        ...
        # 2. 원본 DICOM 정보 가져오기 (spacing 정보용)
        ...
        # 4. 종양 마스크 3D 배열 생성 (binary: 0 또는 1)
        mask_3d = (pixel_array > 0).astype(np.uint8)
        
        # 5. Spacing 정보 추출
        spacing = [1.0, 1.0, 1.0]  # 기본값
        if reference_dicom:
            if hasattr(reference_dicom, 'PixelSpacing') and reference_dicom.PixelSpacing:
                spacing[0] = float(reference_dicom.PixelSpacing[0])
                spacing[1] = float(reference_dicom.PixelSpacing[1])
            if hasattr(reference_dicom, 'SliceThickness') and reference_dicom.SliceThickness:
                spacing[2] = float(reference_dicom.SliceThickness)
            elif hasattr(reference_dicom, 'SpacingBetweenSlices') and reference_dicom.SpacingBetweenSlices:
                spacing[2] = float(reference_dicom.SpacingBetweenSlices)
        
        # 6. 종양 복셀 좌표 추출 (성능을 위해 다운샘플링 가능)
        tumor_voxels = np.where(mask_3d > 0)
        
        # 7. JSON 직렬화 가능한 형태로 변환
        # 다운샘플링 (너무 많은 복셀 방지)
        downsample_factor = 1
        if len(tumor_voxels[0]) > 100000:
            downsample_factor = 2
            mask_3d_downsampled = mask_3d[::downsample_factor, ::downsample_factor, ::downsample_factor]
            tumor_voxels = np.where(mask_3d_downsampled > 0)
            spacing = [s * downsample_factor for s in spacing]
        
        x_coords = tumor_voxels[0].tolist()
        y_coords = tumor_voxels[1].tolist()
        z_coords = tumor_voxels[2].tolist()
        
        # 스케일링 (실제 물리적 크기)
        x_scaled = [x * spacing[0] for x in x_coords]
        y_scaled = [y * spacing[1] for y in y_coords]
        z_scaled = [z * spacing[2] for z in z_coords]
        
        logger.info(f"✅ 종양 3D 데이터 추출 완료: {len(x_coords)} 복셀")
        ...
        return Response({
            ...
            'z': z_scaled,
            'spacing': spacing,
            'dimensions': [mask_3d.shape[0], mask_3d.shape[1], mask_3d.shape[2]],
            'downsample_factor': downsample_factor,
        })
        
    except Exception as e:
        logger.error(f"❌ 종양 3D 데이터 추출 실패: {str(e)}", exc_info=True)
        return Response({
            'success': False,
            'error': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
