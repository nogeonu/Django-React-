...
import { type Patient, type ImageWithAnalysis } from "@shared/schema";

export default function MedicalImages() {
  const [selectedPatient, setSelectedPatient] = useState<string>("");
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedImage, setSelectedImage] = useState<ImageWithAnalysis | null>(null);
  ...
  const { data: images = [], isLoading } = useQuery({
    queryKey: ["/api/patients", selectedPatient, "images"],
  ...
  const uploadImageMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/patients", selectedPatient, "images"] });
    },
    onError: () => {
      toast({
        title: "업로드 실패",
  ...
  const aiAnalysisMutation = useMutation({
    mutationFn: async ({ imageId, imageData }: { imageId: string; imageData: string }) => {
      const response = await apiRequest("POST", "/api/ai-analysis", { imageId, imageData });
      return response.json();
    },
    onSuccess: () => {
      toast({
    ...
    try {
      ...
      reader.onload = async (e) => {
        if (e.target?.result) {
          ...
          const compressedImage = await compressImage(imageData, 0.8, 1024);
          
          const uploadData = {
            patientId: selectedPatient,
            imageType: "MRI", // 기본값, 실제로는 사용자가 선택
            bodyPart: "Head", // 기본값, 실제로는 사용자가 선택
  ...
  const handleAiAnalysis = async (image: ImageWithAnalysis) => {
    try {
      const base64Data = dataURLToBase64(image.imageUrl);
  ...
  const getAnalysisStatusBadge = (image: ImageWithAnalysis) => {
    if (image.analysis) {
      return (
  ...
  const filteredImages = (images as ImageWithAnalysis[]).filter((image: ImageWithAnalysis) =>
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredImages.map((image: ImageWithAnalysis) => (
                    <div
                      key={image.id}
                    ...
                    >
                      <div className="aspect-video bg-gray-100 relative">
                        <img
                          src={image.imageUrl}
                          alt={`${image.imageType} - ${image.bodyPart}`}
                      ...
                      <div className="p-4">
                        <h3 className="font-medium text-gray-900 mb-1" data-testid={`text-image-type-${image.id}`}>
                          {image.imageType} - {image.bodyPart}
                        </h3>
                        ...
                        {image.analysis && (
                          <div className="mb-3">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span className={`text-xs font-medium ${getConfidenceColor(image.analysis.confidence || 0)}`}>
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-lg font-semibold">
                  {selectedImage.imageType} - {selectedImage.bodyPart}
                </h3>
                <Button
                  variant="outline"
                  size="sm"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <img
                    ...
                    alt={`${selectedImage.imageType} - ${selectedImage.bodyPart}`}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span>{selectedImage.imageType}</span>
                      </div>
                      <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>{new Date(selectedImage.uploadedAt || '').toLocaleDateString('ko-KR')}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          ...
                          <span className={`font-medium ${getConfidenceColor(selectedImage.analysis.confidence || 0)}`}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-900">{selectedImage.analysis.findings}</p>
                        </div>
                        
                        <div>
                          <span className="text-sm text-gray-600 block mb-1">권장사항:</span>
                          <p className="text-sm text-gray-900">{selectedImage.analysis.recommendations}</p>
                        </div>
                        
                        <div className="flex justify-between text-xs text-gray-500">
                          <span>분석일: {new Date(selectedImage.analysis.analysisDate || '').toLocaleDateString('ko-KR')}</span>
                          <span>모델: {selectedImage.analysis.modelVersion}</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex space-x-3 pt-4">
                    ...
                    >
                      ...
                      {selectedImage.analysis ? "재분석" : "AI 분석"}
                    </Button>
