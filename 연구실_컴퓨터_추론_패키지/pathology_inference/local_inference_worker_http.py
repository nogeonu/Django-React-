...
def process_request(request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.get('success'):
            ...
            logger.info(f"   - 클래스: {result.get('class_name')}")
            logger.info(f"   - 신뢰도: {result.get('confidence'):.4f}")
            logger.info(f"   - 소요 시간: {elapsed_time:.2f}초")
            logger.info(f"   - 패치 수: {result.get('num_patches')}")
        else:
...
def main():
    """메인 워커 루프"""
    logger.info("="*60)
    logger.info("🚀 병리 이미지 추론 HTTP API 워커 시작")
    logger.info("="*60)
